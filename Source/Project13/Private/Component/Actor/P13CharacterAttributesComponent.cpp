...
void UP13CharacterAttributesComponent::BeginPlay()
{
	...
	Shield = MaxShield;
	Multicast_OnShieldChangedBroadcast(Shield, 0.f, 1.f);
}
...
void UP13CharacterAttributesComponent::ReceiveDamage(const float Damage, AController* Causer)
{
	...
	Multicast_OnShieldChangedBroadcast(Shield, FMath::Min(Damage, ShieldBefore), Shield / MaxShield);
...
void UP13CharacterAttributesComponent::ShieldRecoveryTick(const float DeltaTime)
{
	...
	Multicast_OnShieldChangedBroadcast(Shield, 0.f, Shield / MaxShield);
...
void UP13CharacterAttributesComponent::Multicast_OnShieldChangedBroadcast_Implementation(const float NewShield, const float LastDamage, const float ShieldAlpha)
{
	OnShieldChanged.Broadcast(NewShield, LastDamage, ShieldAlpha);
}
