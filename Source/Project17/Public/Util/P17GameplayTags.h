// Copyright © 2025, Ivan Kinder

#pragma once

#include "NativeGameplayTags.h"

namespace P17::Tags
{
	/// @par Input tags --------------------------------------------------------
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Input_Move)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Input_Look)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Input_Jump)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Input_Roll)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Input_Equip_Axe)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Input_Unequip_Axe)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Input_Attack_Light_Axe)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Input_Attack_Heavy_Axe)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Input_Hold)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Input_Hold_Block)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Input_Toggle)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Input_Toggle_TargetLock)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Input_Toggle_Rage)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Input_SwitchTarget)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Input_Special_Light)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Input_Special_Heavy)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Input_Pickup_Stones)

	/// @par Player tags -------------------------------------------------------
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Player_Ability_Equip_Axe)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Player_Ability_Unequip_Axe)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Player_Ability_Attack_Light_Axe)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Player_Ability_Attack_Heavy_Axe)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Player_Ability_Hit_Pause)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Player_Ability_Roll)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Player_Ability_Block)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Player_Ability_TargetLock)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Player_Ability_Rage)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Player_Ability_Special_Light)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Player_Ability_Special_Heavy)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Player_Ability_Pickup_Stones)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Player_Event_Equip_Axe)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Player_Event_Unequip_Axe)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Player_Event_Hit_Pause)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Player_Event_Hit_SuccessfulBlock)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Player_Event_SwitchTarget_Left)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Player_Event_SwitchTarget_Right)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Player_Event_Rage_Activating)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Player_Event_Ultimate)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Player_Event_Consume_Stones)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Player_Weapon_Axe)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Player_Cooldown_Special_Light)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Player_Cooldown_Special_Heavy)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Player_Status_JumpToFinisher)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Player_Status_Rolling)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Player_Status_Blocking)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Player_Status_TargetLock)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Player_Status_Rage_Activating)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Player_Status_Rage_Active)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Player_Status_Rage_Full)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Player_Status_Rage_None)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Player_SetByCaller_AttackType_Light)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Player_SetByCaller_AttackType_Heavy)

	/// @par Enemy tags --------------------------------------------------------
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Enemy_Ability_Melee)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Enemy_Ability_Range)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Enemy_Ability_Summon)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Enemy_Ability_Spawn_Stone)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Enemy_Weapon)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Enemy_Status_Strafing)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Enemy_Status_UnderAttack)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Enemy_Status_Unblockable)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Enemy_Event_ActiveFist_Left)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Enemy_Event_ActiveFist_Right)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Enemy_Event_ActiveFist_Both)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Enemy_Event_Summon)

	/// @par Shared tags -------------------------------------------------------
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Shared_Ability_Hit_React)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Shared_Ability_Death)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Shared_Event_Hit_Melee)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Shared_Event_Hit_React)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Shared_Event_Spawn_Projectile)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Shared_SetByCaller_Damage_Base)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Shared_Status_Dead)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Shared_Status_HitReact_Front)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Shared_Status_HitReact_Back)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Shared_Status_HitReact_Left)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Shared_Status_HitReact_Right)
	PROJECT17_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Shared_Status_Invincible)
}
